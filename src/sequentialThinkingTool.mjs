...
import { SequentialThinkingServer } from '@modelcontextprotocol/server-sequential-thinking/dist/lib.js';
...
const parseStructuredContent = (result) => {
  ...
  if (contentItem.type !== 'text') {
    throw new TypeError('Sequential Thinking returned a non-text content item.');
  }

  return JSON.parse(contentItem.text);
};

export const createSequentialThinkingState = () => {
  return new SequentialThinkingServer();
};

export const registerSequentialThinkingTool = (server, thinkingServer) => {
  server.registerTool(
    'sequentialthinking',
    {
    ...
    async (arguments_) => {
      const result = thinkingServer.processThought(arguments_);
      ...
      return {
        ...
        structuredContent: parseStructuredContent(result)
      };
    }
  );
};
